...
PROTOCOL='RLS-IBKR-WAYBACK-SNAPSHOT-SEMANTIC-VALIDATION-2025-06-18-v1.2'
...
def numeric_or_missing(s,label):
 s=s.strip()
 if s in MISSING_SENTINELS:return None
...
def main():
 req=urllib.request.Request(URL,headers={'User-Agent':'RLS-Build-Colony-SemanticValidation/1.2'})
 ...
 seen=set();paired=0;obs_avail=0;obs_unavail=0;missing_avail=0;missing_fee=0;missing_rebate=0;figi_nonblank=0;na_fee=0;na_rebate=0
 ...
 for i,line in enumerate(body,1):
  ...
  seen.add(con)
  rebate_s=rebate.strip();fee_s=fee.strip();rebate_n=numeric_or_missing(rebate,f'row {i} REBATERATE'); fee_n=numeric_or_missing(fee,f'row {i} FEERATE'); av=availability(avail,f'row {i} AVAILABLE')
  if rebate_s=='NA':na_rebate+=1
  if fee_s=='NA':na_fee+=1
  ...
  if rebate_n is None:missing_rebate+=1
 ...
 out={
  ...
  'paired_fee_availability_row_count':paired,'observed_available_row_count':obs_avail,'observed_unavailable_row_count':obs_unavail,'missing_availability_row_count':missing_avail,'missing_fee_row_count':missing_fee,'missing_rebate_row_count':missing_rebate,'na_fee_row_count':na_fee,'na_rebate_row_count':na_rebate,'nonblank_figi_row_count':figi_nonblank,
  'missing_sentinel_binding':{'REBATERATE':['','NA'],'FEERATE':['','NA']},'na_coerced_to_zero':False,
 ...
 out['evidence_digest_sha256']=digest(dict(out));os.makedirs('workloads/artifacts',exist_ok=True);json.dump(out,open('workloads/artifacts/rls_ibkr_wayback_snapshot_semantic_validation_2025_06_18_v1_2.json','w'),indent=2,sort_keys=True)
 for k in ('status','source_sha256','row_count','distinct_contract_count','paired_fee_availability_row_count','observed_available_row_count','observed_unavailable_row_count','missing_availability_row_count','missing_fee_row_count','missing_rebate_row_count','na_fee_row_count','na_rebate_row_count','nonblank_figi_row_count','recovery_precheck_pass','normalized_body_sha256','evidence_digest_sha256'):print(k,'=',out[k])
