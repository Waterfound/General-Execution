...
WORKER_VERSION='RLS-IBKR-DIRECT-WAYBACK-RETRY-WORKER-v1.1'; PROTOCOL='RLS-IBKR-DIRECT-WAYBACK-RETRY-v1.1'; AUTHORITY='OBSERVATION_ONLY_NO_RLS_ADMISSION_AUTHORITY'; ENDPOINT='https://archive.org/wayback/available'
def months(a,b):
...
PAIRS=[('http://ftp3.interactivebrokers.com/usa.txt',a) for a in ALL]+[('https://ftp3.interactivebrokers.com/usa.txt',a) for a in ALL]+[('https://ftp2.interactivebrokers.com/usa.txt',a) for a in FTP2_HTTPS]
def q(p):
 t,a=p;u=ENDPOINT+'?'+urllib.parse.urlencode({'url':t,'timestamp':a});last=None
 for attempt in range(1,5):
  try:
   req=urllib.request.Request(u,headers={'User-Agent':'RLS-Build-Colony-DirectRetry/1.1'})
...
def main():
 assert len(PAIRS)==212 and len(set(PAIRS))==212
